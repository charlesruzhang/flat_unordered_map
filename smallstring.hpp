...
#include <bits/stdc++.h>
#include <string_view>
class alignas(16) smallstring {
    public:
        uint64_t hash;
        char    buf[8];
        smallstring() noexcept : hash(0), buf{} {}

        explicit smallstring(std::string_view s) noexcept {
            uint8_t len = s.size() >= 8 ? 8 : static_cast<uint8_t>(s.size());
            memset(buf, 0, 8);
        ...
        std::string_view get_string() const noexcept {
            return std::string_view(buf, hash >> 56);
        }
...
class smallstringEq {
    public:
        bool operator()(const smallstring& s, const smallstring& t) const noexcept {
            if (s.hash != t.hash) return false;
            return std::memcmp(s.buf, t.buf, (t.hash >> 56)) == 0;
        }
};